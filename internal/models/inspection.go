...
// ============================================================
//
// InspectionRule 描述“对哪个域名、按什么周期、用哪个模型、做什么检查”。
...
type InspectionRule struct {
	...
	DomainID         string    `json:"domain_id" neo4j:"domain_id"`               // 关联域名
	Enabled          bool      `json:"enabled" neo4j:"enabled"`                   // 是否启用（false=暂停）
	Schedule         string    `json:"schedule" neo4j:"schedule"`                 // Cron 表达式（5/6 字段）
	Provider         string    `json:"provider,omitempty" neo4j:"provider"`       // 指定模型供应商（空=默认策略）
	Model            string    `json:"model,omitempty" neo4j:"model"`             // 指定模型名（覆盖供应商默认）
	PromptTemplate   string    `json:"prompt_template,omitempty" neo4j:"prompt_template"`
...
type InspectionRecord struct {
	...
	TriggeredBy   string    `json:"triggered_by" neo4j:"triggered_by"` // schedule | manual
